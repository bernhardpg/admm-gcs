# Taken from https://www.webucator.com/article/python-color-constants-module/
...
class RGB(Color):
    ...
    def __add__(self, other: "RGB") -> "RGB":
        red = (other.red + self.red) / 2
        green = (other.green + self.green) / 2
        blue = (other.blue + self.blue) / 2
        return RGB(red, green, blue)

    def __mul__(self, val: float) -> "RGB":
        if not isinstance(val, float):
            raise ValueError("Can only multiply by float")
    ...
    def __rmul__(self, val: float) -> "RGB":
        if not isinstance(val, float):
            raise ValueError("Can only multiply by float")
